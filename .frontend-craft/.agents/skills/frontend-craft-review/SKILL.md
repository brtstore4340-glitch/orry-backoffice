...
description: 对指定文件或最近变更的前端代码进行规范化评审，输出分级评审报告并保存为 Markdown 文件。当用户要求代码评审、review、审查最近改动时自动激活。
---

# 前端代码评审（含 Git 范围）

对前端代码进行全面评审，支持指定文件或自动获取最近变更。
...
2. **过滤**：只保留前端相关文件（`.ts`, `.tsx`, `.vue`, `.js`, `.jsx`, `.css`, `.scss`, `.less`, `.html`）
