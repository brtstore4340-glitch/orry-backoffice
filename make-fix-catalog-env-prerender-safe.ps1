...
$ScriptPath = Join-Path $ToolsDir "fix-catalog-env-prerender.safe.ps1"
...
Invoke-Step -Name "Force catalog page to dynamic rendering" -Action {
  ...
  if ($content -notmatch "export\s+const\s+dynamic\s*=\s*'force-dynamic'") {
    ...
    Write-Log ("Added force-dynamic to: {0}" -f $CatalogPage)
  } else {
    Write-Log ("Catalog page already uses force-dynamic: {0}" -f $CatalogPage)
  }
}
...
Set-Content -LiteralPath (Join-Path $ToolsDir "LAST_BACKUP_DIR.txt") -Value "" -Encoding utf8

Write-Host "Created script:" $ScriptPath -ForegroundColor Green
Write-Host "Log folder    :" $LogsDir
'@ | Set-Content -LiteralPath "D:\01-Main-Work\Boots\Agentic-AI\mission-control\orry\tools\make-fix-catalog-env-prerender-safe.ps1" -Encoding utf8