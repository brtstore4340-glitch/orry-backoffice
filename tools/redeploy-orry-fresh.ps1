...
$BackupDir = Join-Path $ToolsDir ("backup_redeploy_orry_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
$LastBackupPointer = Join-Path $ToolsDir "LAST_BACKUP_DIR.txt"
$script:LogFile = Join-Path $LogsDir "redeploy-orry.log"
$SummaryFile = Join-Path $LogsDir "redeploy-orry-summary.txt"
$EnvScanFile = Join-Path $LogsDir "redeploy-orry-env-scan.txt"
...
Run-Step -Label "Validate paths and prepare folders" -Action {
    ...
    if (Test-Path -LiteralPath $script:LogFile) {
        Copy-Item -LiteralPath $script:LogFile -Destination (Join-Path $BackupDir "redeploy-orry.previous.log") -Force
    }
...
Run-Step -Label "Normalize .env.local Supabase URL when present" -Action {
    if (Test-Path -LiteralPath $EnvLocal) {
        ...
        $text = @"
NEXT_PUBLIC_SUPABASE_URL=$ExpectedSupabaseUrl
"@
        Write-Utf8NoBomFile -Path $EnvLocal -Content $text
...
Run-Step -Label "Scan environment and config" -Action {
    ...
    $lines.Add(("CLI supabase: {0}" -f ($(if ($hasSupabase) { "FOUND" } else { "MISSING" }))))
    $lines.Add(("CLI psql: {0}" -f ($(if ($hasPsql) { "FOUND" } else { "MISSING" }))))
    $lines.Add(("CLI npm: {0}" -f ($(if ($hasNpm) { "FOUND" } else { "MISSING" }))))

    Write-Utf8NoBomFile -Path $EnvScanFile -Content ($lines -join [Environment]::NewLine)
    Log ("Environment scan written: {0}" -f $EnvScanFile)
}

Run-Step -Label "Verify required commands" -Action {
    foreach ($cmd in @("npm")) {
...
Run-Step -Label "Dry scan deploy script for stale refs" -Action {
    ...
    foreach ($r in @("csulanaivoltmubaktvn","csulanaivoltmubaktvn")) {
        if ($text -match [regex]::Escape($r)) { $stale += $r }
    }
    if ($stale.Count -gt 0) {
...
$summary.Add("ORRY REDEPLOY SUMMARY")
