...
function Find-ImportantFiles {
    ...
    if (Test-Path -LiteralPath $srcRoot) {
        Get-ChildItem -Path $srcRoot -File -Recurse -ErrorAction SilentlyContinue | Where-Object {
...
function Replace-ProjectRefInFile {
    param(
        ...
        [Parameter(Mandatory = $true)][string]$PatchScanFile
    )

    if (-not (Test-Path -LiteralPath $Path)) {
        Add-Content -LiteralPath $PatchScanFile -Value ("SKIP_MISSING | " + $Path) -Encoding utf8
        return $false
    }

    $old = ""
    try {
        $old = Get-Content -LiteralPath $Path -Raw -Encoding UTF8 -ErrorAction Stop
        if ($null -eq $old) { $old = "" }
    } catch {
        Add-Content -LiteralPath $PatchScanFile -Value ("SKIP_READ_ERROR | " + $Path + " | " + $_.Exception.Message) -Encoding utf8
        return $false
    }

    if ([string]::IsNullOrEmpty($old)) {
        Add-Content -LiteralPath $PatchScanFile -Value ("SKIP_EMPTY | " + $Path) -Encoding utf8
        return $false
    }
    ...
    if ($new -ne $old) {
        ...
        Add-Content -LiteralPath $PatchScanFile -Value ("PATCHED | " + $Path) -Encoding utf8
        Log ("Patched project ref in file: {0}" -f $Path)
        return $true
    }

    Add-Content -LiteralPath $PatchScanFile -Value ("UNCHANGED | " + $Path) -Encoding utf8
    return $false
}
...
$BackupDir = Join-Path $ToolsDir ("backup_redeploy_orry_v3_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
$LastBackupPointer = Join-Path $ToolsDir "LAST_BACKUP_DIR.txt"
$script:LogFile = Join-Path $LogsDir "redeploy-orry-v3.log"
$SummaryFile = Join-Path $LogsDir "redeploy-orry-v3-summary.txt"
$EnvScanFile = Join-Path $LogsDir "redeploy-orry-v3-env-scan.txt"
$PatchScanFile = Join-Path $LogsDir "redeploy-orry-v3-patch-scan.txt"
...
Run-Step -Label "Validate paths and prepare folders" -Action {
    ...
    New-EmptyUtf8NoBomFile -Path $PatchScanFile
    Add-Content -LiteralPath $PatchScanFile -Value ("PATCH SCAN START | " + (Get-Date -Format "yyyy-MM-dd HH:mm:ss")) -Encoding utf8
}
...
Run-Step -Label "Patch project ref in important files" -Action {
    ...
    foreach ($file in $files) {
        try {
            if (Replace-ProjectRefInFile -Path $file -NewRef $ProjectRef -PatchScanFile $PatchScanFile) {
                $patchedCount++
            } else {
                $skipCount++
            }
            ...
            $skipCount++
            Add-Content -LiteralPath $PatchScanFile -Value ("SKIP_EXCEPTION | " + $file + " | " + $_.Exception.Message) -Encoding utf8
        }
    }
    Log ("Patched files count: {0}" -f $patchedCount)
    Log ("Skipped/unchanged files count: {0}" -f $skipCount)
}

Run-Step -Label "Normalize .env.local Supabase URL" -Action {
    if (Test-Path -LiteralPath $EnvLocal) {
        $text = Safe-ReadFileText -Path $EnvLocal
...
Run-Step -Label "Scan environment and config" -Action {
    ...
    foreach ($path in @($EnvPlain, $EnvLocal, $DeployScript, $SupabaseConfig, (Join-Path $RepoRoot "package.json"))) {
        ...
        if (Test-Path -LiteralPath $path) {
            $content = Safe-ReadFileText -Path $path
...
Run-Step -Label "Build application" -Action {
    ...
    $pkgText = Safe-ReadFileText -Path $pkgPath
    if ([string]::IsNullOrWhiteSpace($pkgText)) { throw "package.json is empty or unreadable" }
    $pkg = $pkgText | ConvertFrom-Json
    if (-not $pkg.scripts.build) { throw "package.json does not contain a build script" }
...
Run-Step -Label "Dry scan deploy script for stale refs" -Action {
    $text = Safe-ReadFileText -Path $DeployScript
    $stale = @()
...
$summary.Add("ORRY REDEPLOY SUMMARY V3")
...
$summary.Add(("PatchScanFile: {0}" -f $PatchScanFile))
$summary.Add("")
