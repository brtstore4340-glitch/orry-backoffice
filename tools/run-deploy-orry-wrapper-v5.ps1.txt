...
function Write-Summary {
    ...
    $summary.Add("DEPLOY ORRY WRAPPER SUMMARY V5")
...
$BackupDir = Join-Path $ToolsDir ("backup_deploy_wrapper_v5_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
$LastBackupPointer = Join-Path $ToolsDir "LAST_BACKUP_DIR.txt"
$DeployScript = Join-Path $ToolsDir "deploy-orry-direct.ps1"
$script:WrapperLogFile = Join-Path $LogsDir "deploy-orry-wrapper-v5.log"
$ChildLogFile = Join-Path $LogsDir "deploy-orry-child-v5.log"
$SummaryFile = Join-Path $LogsDir "deploy-orry-wrapper-v5-summary.txt"
...
try {
    ...
    Run-Step -Label "Run deploy script with child error capture" -Action {
        $childBootstrap = @'
$ErrorActionPreference = "Stop"
$deployScript = "D:\01-Main-Work\Boots\Agentic-AI\mission-control\orry\tools\deploy-orry-direct.ps1"
$childLog = "D:\01-Main-Work\Boots\Agentic-AI\mission-control\orry\tools\logs\deploy-orry-child-v5.log"
...
'@
    ...
    if ($script:RunSucceeded) {
        Write-Summary -ResultText "SUCCESS" -ExitCode $script:FinalExitCode
    } else {
        Write-Summary -ResultText "FAILED" -ExitCode $script:FinalExitCode
    }
