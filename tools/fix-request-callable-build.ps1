...
$BackupDir  = Join-Path $ToolsDir ("backup_fix_request_callable_{0}" -f $Stamp)
$script:LogFile = Join-Path $LogsDir ("fix-request-callable_{0}.log" -f $Stamp)
...
function Update-JsonExclude {
  ...
  foreach ($item in $existing) {
    $s = [string]$item
    if (-not [string]::IsNullOrWhiteSpace($s) -and -not $merged.Contains($s)) {
      [void]$merged.Add($s)
    }
  }
...
if (-not $NextConfig) {
  throw "No next config file found in repo root."
}
...
Invoke-Step -Name "Patch request import and callable wrapper" -Action {
  ...
  if ($content -notmatch "const\s+localVarRequestCallable\s*=") {
    $pattern = "import \* as http from 'http';"
    if ($content -match [regex]::Escape($pattern)) {
      $replacement = @"
import * as http from 'http';
const localVarRequestCallable: any = ((localVarRequest as any).default ?? (localVarRequest as any));
"@
      $content = $content.Replace($pattern, $replacement)
    } else {
      throw "Anchor not found for inserting localVarRequestCallable."
    }
  }

  $content = $content -replace "(?<![A-Za-z0-9_])localVarRequest\(", "localVarRequestCallable("

  if ($content -ne $original) {
    Write-Utf8NoBomFile -Path $AuthFile -Content $content
    Write-Log ("Patched SDK request callable usage: {0}" -f $AuthFile)
  } else {
    Write-Log ("No content changes were necessary: {0}" -f $AuthFile)
  }
}
...
Invoke-Step -Name "Disable ESLint during production build" -Action {
...
Invoke-Step -Name "Clear .next cache" -Action {
  ...
  if (Test-Path -LiteralPath $NextCache) {
    ...
    Write-Log ("Removed cache: {0}" -f $NextCache)
  } else {
    Write-Log ("Skip remove cache (not found): {0}" -f $NextCache)
  }
}
