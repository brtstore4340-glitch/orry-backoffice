...
$BackupDir = Join-Path $ToolsDir ("backup_deploy_wrapper_v2_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
$LastBackupPointer = Join-Path $ToolsDir "LAST_BACKUP_DIR.txt"
$DeployScript = Join-Path $ToolsDir "deploy-orry-direct.ps1"
$script:LogFile = Join-Path $LogsDir "deploy-orry-direct-wrapper-v2.log"
$SummaryFile = Join-Path $LogsDir "deploy-orry-direct-wrapper-v2-summary.txt"
...
$summary.Add("DEPLOY ORRY DIRECT WRAPPER SUMMARY V2")
